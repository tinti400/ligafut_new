...
type Props = {
  ...
  // Admin: mandar jogador sem lance para o mercado
  valorMercado?: string
...
function getIsoByNationality(nacionalidade?: string | null) {
  const key = normalizeCountryName(nacionalidade)
  return COUNTRY_TO_ISO2[key] || null
}

function getFlagUrl(nacionalidade?: string | null) {
  const iso = getIsoByNationality(nacionalidade)
  if (!iso) return null
...
export default function CardJogadorLeilao({
  ...
  const valorMercadoNum = useMemo(
    () => Math.floor(Number(valorMercado || leilao.valor_atual || 0)),
    [valorMercado, leilao.valor_atual],
  )
  ...
  const flagUrl = getFlagUrl(leilao.nacionalidade)
      ...
      >
        ...
        <div className="relative z-20 flex flex-col p-4 sm:p-5">
          ...
          <div className="mb-4">
            ...
            <div className="h-2 overflow-hidden rounded-full bg-black/45 ring-1 ring-white/10">
              <div className={classNames('h-full rounded-full transition-[width] duration-1000', barraCor)} style={{ width: `${pctRestante}%` }} />
            </div>
          </div>
            ...
            <div>
              ...
              <div className="mt-2 flex flex-wrap items-center gap-2">
                ...
                {leilao.nacionalidade && (
                  <span className="flex items-center gap-1.5 rounded-xl bg-black/35 px-2.5 py-1 text-[11px] font-semibold text-white/85 ring-1 ring-white/10">
                    {flagUrl && flagOk && (
                      <img
                        src={flagUrl}
                        alt={leilao.nacionalidade}
                        className="h-3.5 w-5 rounded-[2px] object-cover"
                        loading="lazy"
                        onError={() => setFlagOk(false)}
                      />
                    )}
                    <span>{leilao.nacionalidade}</span>
                  </span>
                )}
              </div>
            </div>
          ...
          <div className="mt-48 sm:mt-56">
            <div className="rounded-3xl border border-white/15 bg-black/55 p-4 shadow-2xl backdrop-blur-md">
              <div className="truncate text-center text-lg font-black uppercase tracking-wide text-white">{leilao.nome}</div>
            ...
            {!encerrado ? (
              <>
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                  <input
                    ...
                    className={classNames(
                      ...
                      invalido ? 'border-red-400/45 focus:ring-2 focus:ring-red-400/30' : 'border-emerald-400/30 focus:ring-2 focus:ring-emerald-400/30',
                    )}
                    value={valorProposto}
                ...
                <div className="mt-3 flex flex-col gap-2 text-[11px] text-white/75 sm:flex-row sm:items-center sm:justify-between">
                  <span>
                    Mínimo: <b className="tabular-nums text-white">{brl(minimoPermitido)}</b>
                  </span>
              ...
              <>
                {hasVencedor && isAdmin && onFinalizar ? (
                  ...
                  <div className="rounded-2xl border border-emerald-400/20 bg-emerald-400/10 p-3">
                    <div className="text-xs font-black uppercase tracking-[0.16em] text-emerald-200">Admin • Mandar para o mercado</div>
        ...
        {burst && (
          <div className="pointer-events-none absolute inset-0 z-40 flex items-center justify-center">
            <div className="animate-[fadeout_0.75s_ease_forwards] select-none text-5xl">💥✨🔥</div>
          </div>
        )}
