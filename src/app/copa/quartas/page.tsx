...
export default function QuartasPage() {
  ...
  const quartasCompletas = useMemo(() => {
    if (jogos.length !== 4 && jogos.length !== 8) {
      // usualmente 4 confrontos; mas deixo flexível
      // se não souber, apenas exige que todos campos existentes estejam preenchidos
    }
    return jogos.length > 0 && jogos.every(j =>
      j.gols_time1 !== null &&
      j.gols_time2 !== null &&
  ...
  async function finalizarQuartas() {
    ...
    try {
      ...
      const ins1 = await supabase
        .from('copa_semi')
        .insert(semi.map(s => ({ ...s, gols_time1_volta: null, gols_time2_volta: null })))
      if (ins1.error && mentionsVolta(ins1.error.message)) {
        const ins2 = await supabase.from('copa_semi').insert(semi)
  ...
  return (
    <div className="relative p-4 sm:p-6 max-w-7xl mx-auto">
      {/* brilhos suaves de fundo (igual referência) */}
      ...
      {/* Lista de jogos com o mesmo card/layout das Oitavas */}
...
/** ====== Cartão de Jogo (mesmo visual do arquivo de Oitavas) ====== */
function MatchCard({
