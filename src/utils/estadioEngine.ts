...
export type EstadioContext = {
  ...
  infraScore: number // 0..100 (cobertura, banheiro, segurança, etc. pode ser um agregado)
  level: number // nível do estádio 1..NIVEL_MAXIMO
}
...
type Segment = {
  id: SegmentId
  share: number // fração da demanda potencial
  priceK: number // sensibilidade ao preço (maior = mais sensível)
  comfortK: number // quanto liga pra conforto/infra
...
export function simulate(
  ...
  // demanda potencial por setor (somando segmentos)
  ...
  ;(Object.keys(seats) as Sector[]).forEach((sector) => {
    ...
    for (const seg of segments) {
      const A = base * seg.share * fanBase * seg.attraction[sector] // interesse bruto no setor
      // efeito do preço (exponencial decrescente) + conforto
      const priceEffect = Math.exp(-seg.priceK * (pRatio - 0.9)) // p≈ref → ~e^{-k*0.1} próximo de 0.9..1
  ...
  // Alocação real: capacidade por setor menos sócios
  ...
  ;(Object.keys(seats) as Sector[]).forEach((sector) => {
    ...
    const revenueSoc = socios * clamp(ctx.sociosPreco, 0, price) // sócios pagam preço fixo (ou 0 se for gratuidade)
...
export function optimizePrices(
  ...
  ctx: EstadioContext,
  mode: OptimizeMode,
  targetOcc: number = 0.92 // 92% de meta
): PriceMap {
  const ref = referencePrices(ctx.level)
  const lim = priceLimits(ctx.level)

  // grade de busca em volta da referência
  ...
  // 2 passes de coordinate descent simples
  let basePrices = { ...current }
  for (let pass = 0; pass < 2; pass++) {
    for (const s of sectors) {
      let localBestPrice = basePrices[s]
      ...
      for (const m of multipliers) {
        ...
        const score =
          mode === 'maxProfit'
            ? sim.profit
            : -Math.abs(sim.occupancy - targetOcc) * 1e6 + sim.profit * 0.001 // prioriza bater meta, desempata por lucro

        if (score > localBestScore) {
          localBestScore = score
          localBestPrice = trial[s]
        }
      }
      basePrices[s] = localBestPrice
    }
  }

  // avaliação final
  const finalSim = simulate(capacity, basePrices, ctx)
  best = basePrices
  bestScore = mode === 'maxProfit' ? finalSim.profit : -Math.abs(finalSim.occupancy - targetOcc)
