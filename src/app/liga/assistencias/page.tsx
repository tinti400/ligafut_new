...
import { FiRefreshCw, FiTarget, FiTrendingUp, FiAward } from "react-icons/fi";
...
function fotoJogador(j?: Jogador | null) {
  return j?.imagem_url || j?.foto || "/default-player.png";
}
...
export default function LigaAssistenciasPage() {
  ...
  async function carregar() {
    ...
    if (error) {
      console.error("Erro ao carregar assistências da Liga:", error);
    }
  ...
  const ranking = useMemo(() => {
    const map: Record<string, LinhaAssistencia> = {};
    ...
    for (const jogo of jogos) {
      ...
      for (const evento of eventos) {
        ...
        const nomeAssistencia = evento.assistencia || null;

        if (!assistenciaId && !nomeAssistencia) continue;

        const idTime = evento.time_id || null;
        const key = assistenciaId || `${idTime || "sem_time"}:${nomeAssistencia}`;

        const jogadorBanco = assistenciaId ? jogadoresMap[assistenciaId] : null;
        ...
        if (!map[key]) {
          map[key] = {
            ...
            nome: jogadorBanco?.nome || nomeAssistencia || "Jogador",
  ...
  return (
    <div className="w-full max-w-full overflow-x-hidden px-3 py-4 text-zinc-100 space-y-6 sm:px-4 md:px-6">
      <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-sky-500/10 via-white/[0.04] to-emerald-500/10 p-5 shadow-2xl">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-sm text-zinc-400">
              Assistências registradas automaticamente nos gols da Liga — temporada {TEMPORADA}
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Link
              href="/liga/artilharia"
              className="rounded-xl bg-yellow-500/20 px-4 py-2 text-sm font-black text-yellow-300 hover:bg-yellow-500/30"
            >
              Artilharia
            </Link>
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          <div className="rounded-xl border border-white/10 bg-black/25 p-4">
            <div className="text-xs text-zinc-400">Garçom da Liga</div>
          ...
          <div className="rounded-xl border border-white/10 bg-black/25 p-4">
            <div className="text-xs text-zinc-400">Assistências do líder</div>
            <div className="mt-1 text-lg font-black">{lider?.assistencias || 0}</div>
          </div>
      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
        <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            <h2 className="flex items-center gap-2 text-xl font-black">
              <FiTrendingUp /> Ranking de Assistências
            </h2>
            <p className="text-sm text-zinc-400">
              Cada assistência valoriza o atleta em +0,025% no momento em que o placar é salvo.
            </p>
          </div>
        ...
        {loading ? (
          <div className="py-10 text-center text-zinc-400">Carregando assistências...</div>
        ) : ranking.length === 0 ? (
          <div className="rounded-xl border border-white/10 bg-black/25 p-8 text-center text-zinc-400">
            Nenhuma assistência encontrada ainda. Nem todo gol tem assistência.
          </div>
        ) : (
                ...
                >
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-xs text-zinc-400">
                      assist. em {linha.jogos.size} jogo(s)
                    </div>
                  </div>
                </div>
              );
