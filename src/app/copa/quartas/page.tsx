...
export default function SemiPage() {
  ...
  async function buscarJogos() {
    ...
    // 1) tenta com *_volta e com 'ordem'
    ...
    // 2) se deu erro de *_volta, tenta sem *_volta (ainda com 'ordem')
    ...
    // 3) se ainda deu erro e for por falta de 'ordem', tenta SEM 'ordem' (ordenando por id)
    if (error && mentionsOrdem(error.message)) {
      // tenta novamente incluindo *_volta, mas sem pedir/ordenar por 'ordem'
      const q3 = await supabase
        .from('copa_semi')
        .select('id,id_time1,id_time2,time1,time2,gols_time1,gols_time2,gols_time1_volta,gols_time2_volta')
        .order('id', { ascending: true })
      if (q3.error && mentionsVolta(q3.error.message)) {
        setSupportsVolta(false)
        const q4 = await supabase
          .from('copa_semi')
          .select('id,id_time1,id_time2,time1,time2,gols_time1,gols_time2')
          .order('id', { ascending: true })
        data = q4.data as any
        error = q4.error
      } else {
  ...
  async function finalizarSemi() {
    ...
    // recomputa vencedores e popula a Final (tenta lista de times, depois partida única)
