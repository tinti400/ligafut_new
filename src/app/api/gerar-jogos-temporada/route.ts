...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // (opcional) limpar registros existentes desta temporada
    if (limparExistentes) {
    ...
    // times + divisão (number ou string)
    ...
    const dados = times
      ...
      .map((t) => ({
        ...
        gols_contra: 0,
        jogos: 0,
        // saldo: 0, // ⚠️ removido para não quebrar caso a coluna não exista
      }))
    ...
    // Use o UNIQUE que combina com seu schema (geralmente id_time+temporada+divisao)
    ...
    if (errorInsert) {
      // exponha a mensagem real para facilitar o debug no Network tab
      return NextResponse.json({ erro: errorInsert.message }, { status: 500 })
    }
