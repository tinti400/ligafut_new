...
import { useEffect, useMemo, useState, useCallback } from 'react'import type { ReactNode } from 'react'import Link from 'next/link'import { usePathname, useRouter } from 'next/navigation'import { createClient } from '@supabase/supabase-js'
...
const safe = (v: any) => {const n = Number(v ?? 0)return Number.isFinite(n) ? n : 0}

const fmtBRL0 = (n: number) =>n.toLocaleString('pt-BR', {style: 'currency',currency: 'BRL',maximumFractionDigits: 0,})
...
async function checarAdmin(pIdent: string) {const ident = String(pIdent || '').trim().toLowerCase()if (!ident) return false

const { data, error } = await supabase.rpc('is_admin', { p_email: ident })if (error) return false

return data === true}

function pegarIdentidadeLocalStorage(): { email: string; usuario: string } {const tryParse = (k: string) => {const s = localStorage.getItem(k)if (!s) return nulltry {return JSON.parse(s)} catch {return null}}

const u1 = tryParse('user')const u2 = tryParse('usuario')const u3 = tryParse('auth')
...
const email = String(u1?.email ||u2?.email ||u3?.email ||rawEmail ||u1?.usuario ||u2?.usuario ||'').trim()

const usuario = String(u1?.usuario ||u2?.usuario ||rawUsuario ||u1?.login ||u2?.login ||'').trim()

return { email, usuario }}

export default function Sidebar() {const router = useRouter()const pathname = usePathname()

const [isOpen, setIsOpen] = useState(true)const [headerVisible, setHeaderVisible] = useState(true)

const [abrirClube, setAbrirClube] = useState(true)const [abrirCompeticoes, setAbrirCompeticoes] = useState(true)const [abrirMercado, setAbrirMercado] = useState(true)const [abrirAdmin, setAbrirAdmin] = useState(false)

const [checkingAdmin, setCheckingAdmin] = useState(true)const [isAdmin, setIsAdmin] = useState(false)

const [logado, setLogado] = useState(false)const [idTime, setIdTime] = useState<string | null>(null)const [nomeTime, setNomeTime] = useState('')const [logoUrl, setLogoUrl] = useState<string | null>(null)

const [saldoTime, setSaldoTime] = useState(0)const [moedas, setMoedas] = useState(0)const [totalSalarios, setTotalSalarios] = useState(0)const [dividaEmprestimos, setDividaEmprestimos] = useState(0)const [valorParcela, setValorParcela] = useState(0)const [parcelasRestantes, setParcelasRestantes] = useState<number | null>(null)

const [countRecebidas, setCountRecebidas] = useState(0)const [countEnviadas, setCountEnviadas] = useState(0)

const isActive = useCallback((href: string) => {if (!pathname) return falseif (href === '/') return pathname === '/'return pathname === href || pathname.startsWith(href + '/')},[pathname])

useEffect(() => {try {const open = localStorage.getItem('sb_open')if (open !== null) setIsOpen(open === '1')
...
useEffect(() => {try {localStorage.setItem('sb_open', isOpen ? '1' : '0')} catch {}}, [isOpen])

const persistGroup = (key: GroupKey, open: boolean) => {try {localStorage.setItem(sb_g_${key}, open ? '1' : '0')} catch {}}

const setHeaderPersist = (v: boolean) => {setHeaderVisible(v)try {localStorage.setItem('sb_header_visible', v ? '1' : '0')} catch {}}

useEffect(() => {const getIdTime = (): string | null => {const direct = localStorage.getItem('id_time') || localStorage.getItem('time_id')if (direct) return direct
...
const getFirstNumber = (row: any, keys: string[]) => {for (const k of keys) {if (row?.[k] != null && Number(row[k]) >= 0) return safe(row[k])}return 0}

useEffect(() => {;(async () => {let timeRow: any = null
...
const logout = () => {try {localStorage.clear()} catch {}

router.push('/login')

}

const saldoFmt = useMemo(() => fmtBRL0(safe(saldoTime)), [saldoTime])const salariosFmt = useMemo(() => fmtBRL0(safe(totalSalarios)), [totalSalarios])const moedasFmt = useMemo(() => fmtInt(safe(moedas)), [moedas])const dividaFmt = useMemo(() => fmtBRL0(safe(dividaEmprestimos)), [dividaEmprestimos])const parcelaFmt = useMemo(() => fmtBRL0(safe(valorParcela)), [valorParcela])

const Badge = ({ n, tone = 'emerald' }: { n: number; tone?: 'emerald' | 'amber' | 'rose' }) => {if (!n || n <= 0) return null
...
const NavLink = ({href,children,badge,}: {href: stringchildren: ReactNodebadge?: ReactNode}) => {const active = isActive(href)
...
const ToggleGroup = ({open,setOpen,storageKey,label,icon,}: {open: booleansetOpen: (v: boolean) => voidstorageKey: GroupKeylabel: stringicon: ReactNode}) => (<buttontype="button"onClick={() => {const v = !opensetOpen(v)persistGroup(storageKey, v)}}className={['w-full flex items-center justify-between px-3 py-2 rounded-xl transition ring-1 ring-inset',open ? 'bg-white/7 ring-white/12' : 'hover/7 ring-white/10',].join(' ')}>{icon}{label}
...
const CollapsedItem = ({href,label,emoji,badge,}: {href: stringlabel: stringemoji: stringbadge?: ReactNode}) => {const active = isActive(href)
...
const HeaderBar = () => (<divclassName="fixed left-0 right-0 z-50 transition-transform duration-300 bg-[#07111f]/80 backdrop-blur-xl border-b border-white/10 shadow-[0_14px_40px_rgba(0,0,0,0.28)]"style={{height: HEADER_H,top: 0,transform: headerVisible ? 'translateY(0)' : translateY(-${HEADER_H}px),}}>LF
...
const HeaderReveal = () =>headerVisible ? null : (<buttontype="button"onClick={() => setHeaderPersist(true)}className="fixed top-2 right-3 z-50 h-10 w-10 rounded-xl grid place-items-center bg-[#07111f]/80 hover/15 ring-1 ring-white/15 text-white backdrop-blur shadow-lg">▼)
